...
5. Compile the program. Windows requires explicitly linking  the underlying OS graphics and media subsystems:
   ...
   ```bash
   g++ main.cpp -o visualiser.exe -O3 -Wall -lraylib -lopengl32 -lgdi32 -lwinmm
...
2. Navigate to the project directory and compile:
   ...
   ```bash
   g++ main.cpp -o visualiser -O3 -Wall -lraylib -lGL -lm -lpthread -ldl -lrt -lX11
   ```
...
2. Navigate to the project directory. macOS requires linking Apple's native rendering frameworks:
   ...
   ```bash
   clang++ main.cpp -o visualiser -O3 -Wall -lraylib -framework CoreVideo -framework IOKit -framework Cocoa -framework OpenGL
   ```
3. Run the executable:
...
Once the hardware link is live (indicated by the green status bar), press [C] to initiate the Multi-Point Calibration Wizard. Follow the on-screen prompts and press [ENTER] to lock in the low-, mid-, and high-weight hardware curve. Press [R] at any time to clear the mapping and reset to default.
