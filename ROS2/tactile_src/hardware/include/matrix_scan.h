...
typedef struct {
	...
	void (*stop_scan_func)(void);
	void (*wait_ready_func)(uint8_t stage);
...
/**
 * @brief Bridge function to read a row at once.
 * @param row Pointer to location of first element to be modified.
 * @param cdc_id is 0 if using cdc_a, 1 if using cdc_b.
 */
void get_row(uint16_t *row, uint8_t cdc_id);
