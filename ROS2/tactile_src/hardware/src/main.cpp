...
/**
 * @brief Bridge function to read the currently selected CDC A sensor.
 * @retval 16-bit value read from CDC A.
 */
extern "C" uint16_t read_analog(void) {
	uint16_t val_a = 0;
	uint16_t val_b = 0;
...
/**
 * @brief Bridge function to read one full AD7142 row from the selected CDC.
 * @param row Pointer to the first value to be updated.
 * @param cdc_id Selects the CDC to read: 0 for CDC A, non-zero for CDC B.
 */
extern "C" void get_row(uint16_t *row, uint8_t cdc_id) {
	alignas(2) uint16_t tx_buf[2];
	alignas(2) uint16_t rx_buf[2] = {0, 0};
	...
	for (uint16_t i = 0; i < 8; ++i) {
		tx_buf[0] = (uint16_t)(0xE400 | ((0x00B + i) & 0x03FF));
		tx_buf[1] = 0x0000;
		rx_buf[0] = 0;
		rx_buf[1] = 0;
...
void trigger_AD7142(void) {
	HAL_Delay(1);
	AD7142_Write_Reg(&hspi1, GPIOA, GPIO_PIN_4, 0x002, 0x8FF0);
	AD7142_Write_Reg(&hspi1, GPIOA, GPIO_PIN_4, 0x000, 0x0170);
	
	if (cdc_b_hardware_present) {
		AD7142_Write_Reg(&hspi2, GPIOB, GPIO_PIN_12, 0x002, 0x8FF0);
		AD7142_Write_Reg(&hspi2, GPIOB, GPIO_PIN_12, 0x000, 0x0170);
	}
}

/**
 * @brief Tells the AD7142 to stop. Call this function when receiving the stage7 interrupt.
 */
void stop_AD7142(void){
	HAL_Delay(13);

	AD7142_Write_Reg(&hspi1, GPIOA, GPIO_PIN_4, 0x000, 0x0171);
	if(cdc_b_hardware_present){
		AD7142_Write_Reg(&hspi2, GPIOB, GPIO_PIN_12, 0x000, 0x0171);
	}
 ...
 */
void wait_for_AD7142(uint8_t stage) {
	uint16_t int_stage;

	while(true){
		if(cdc_a_conversion_complete){
			int_stage = AD7142_Read_Reg(&hspi1, GPIOA, GPIO_PIN_4, 0x00A);
			if(int_stage & (1 << stage)){
				break;
			}
		}
	}
...
int main(void) {
	...
	skin_config.stop_scan_func = stop_AD7142;
...
void AD7142_Init(void) {
	...
	ad7142_reg_config_t bank1_table[] = {
		...
		{0x007, 0x00FF}  // STAGE_COMPLETE_INT_EN
	};
