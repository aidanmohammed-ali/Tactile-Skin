...
def draw_dataset_item(
    ...
    if confidence is None and isinstance(label.get("detection"), dict):
        confidence = label["detection"].get("confidence")
    conf_text = "conf=nan" if confidence is None else f"conf={float(confidence):.2f}"

    lines = [
        f"{index + 1}/{total}  {sample_id}  {label.get('schema_version', 'legacy')}",
        f"{pos_text}  {pose_text}  {conf_text}",
