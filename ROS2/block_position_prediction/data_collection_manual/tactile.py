...
class ThreadedTactileReader:
    ...
    def __init__(
        ...
        frame_callback: Callable[[TactileSnapshot], None] | None = None,
    ) -> None:
        self.port = str(port)
    ...
    def capture_new_raw_frames(
        ...
        if frame_count > CAPTURE_FRAMES:
            raise ValueError(
                f"frame count cannot exceed capture buffer size "
                f"{CAPTURE_FRAMES}"
            )

        deadline = time.monotonic() + max(0.0, float(timeout))
        with self._frame_condition:
            target_sequence = self._frame_sequence + frame_count
            while self._frame_sequence < target_sequence:
            ...
            return tuple(
                frame.copy()
                for frame in self._recent_raw_frames[-frame_count:]
            )
        ...
        while not self._stop.is_set():
            ...
            if raw_frame is not None and raw_frame.size == NUM_TAXELS:
                ...
                if self.frame_callback is not None:
                    self.frame_callback(self.snapshot())
            elapsed = time.perf_counter() - started
            if not self._hardware_requested and elapsed < interval:
                time.sleep(interval - elapsed)
            elif self._hardware_requested and raw_frame is None:
                time.sleep(0.001)
